...
public class GrainsRVAdapter extends RecyclerView.Adapter<GrainsRVAdapter.MyViewHolder> {
    ...
    public GrainsRVAdapter(Context applicationContext, List<ModelGrains> modelGrainsList) {

        this.modelGrainsList = modelGrainsList;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        final ModelGrains modelGrains = modelGrainsList.get(position);

        int itemViewType = getItemViewType(position);
        if (itemViewType == ROW_ITEM) {


            if (Integer.parseInt(modelGrains.getOfferPrice()) == 0) {
                holder.cvGrainsOfferPercentage.setVisibility(View.GONE);
                holder.tvGrainsPrice.setVisibility(View.GONE);
            } else {
                holder.tvGrainsOfferPer.setText(modelGrains.getOfferPrice() + "%" + " OFF");
                holder.tvGrainsPrice.setText(context.getResources().getString(R.string.currency) + " " + modelGrains.getProductPrice());
                holder.tvGrainsPrice.setPaintFlags(holder.tvGrainsPrice.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
            }

            holder.tvGrainsName.setText(modelGrains.getProductEname());

            holder.tvGrainsPrice.setText(context.getResources().getString(R.string.currency) + " " + modelGrains.getProductPrice());
            holder.tvGrainsPrice.setPaintFlags(holder.tvGrainsPrice.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);

            holder.tvGrainsQty.setText(modelGrains.getQty() + " " + modelGrains.getProductWeight());
            holder.tvGrainOfferPrice.setText(context.getResources().getString(R.string.currency) + " " + modelGrains.getFinalPrice());

            if (!modelGrains.getProductImg().equalsIgnoreCase("")) {
                Picasso.with(context)
                        .load(context.getResources().getString(R.string.images_base_url) + modelGrains.getProductImg())
...
//        return ROW_ITEM;
        if (modelGrainsList.get(position) != null) return ROW_ITEM;
        ...
        public MyViewHolder(View itemView) {
            ...
            btnAdd.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ModelGrains current = modelGrainsList.get(getAdapterPosition());
